...
fn find_service<'a>(file: &'a FileDescriptorProto, name: &str) -> &'a ServiceDescriptorProto {
    file.service
        .iter()
...
fn find_method<'a>(svc: &'a ServiceDescriptorProto, name: &str) -> &'a MethodDescriptorProto {
    svc.method
        .iter()
        .find(|m| m.name.as_deref() == Some(name))
...
fn find_enum_value<'a>(e: &'a EnumDescriptorProto, name: &str) -> &'a EnumValueDescriptorProto {
    e.value
        .iter()
